...
cd "/Users/xuwenli/Downloads/did_multiplegt_dyn"


*=====================================================================
* 例子：标准DID——二值型处理，吸收：[1]许文立 & 孙磊.(2023).市场激励型环境规制与能源消费结构转型
* ——来自中国碳排放权交易试点的经验证据.数量经济技术经济研究,40(07),133-155 
*=====================================================================
******************************
****** 许文立和孙磊（2023） ******
******************************

//图 5 de Chaisemartin和D'Haultfoeuille（2022）的事件研究图//
use 202305_EnergyStructure_Robustdata.dta,clear

* 处理示意图
panelview TREnergy CBMarket,i(province_) t(year) type(treat)


***没有协变量
did_multiplegt_dyn TREnergy province_ year NCBMarket,effects(6) placebo(6) cluster(province_)

***有协变量
did_multiplegt_dyn TREnergy province_ year NCBMarket,effects(6) placebo(6) controls(PGDP INSTR FINV FREV CAROWN PFOREST FDI DENS) cluster(province_)




*=====================================================================
* 例子：政策实施后退出：田淑英,孙磊,许文立 & 
* 范子英.(2022).绿色低碳发展目标下财政政策促进企业转型升级研究
* ——来自"节能减排财政政策综合示范城市"试点的证据.财政研究,(08),79-96. 
*=====================================================================
******************************
****** 田淑英等（2022） ******
******************************
use 202208_GreenFinancial_2017_data.dta,clear
...
* 处理示意图
panelview TFP_LP Policy, i(scode) t(year) type(treat)

* 没有协变量
did_multiplegt_dyn TFP_LP scode year Policy, effects(3) placebo(3) cluster(scode)

* 有协变量
did_multiplegt_dyn TFP_LP scode year Policy, effects(3) placebo(3) controls($Xs) cluster(scode)
...
*========================================================================================
* Examples for Several Treatments: Bradford et al. (2022), Rico-Straffon et al.  (2023),赵仁杰等（2023） *
*========================================================================================
**********************************
* 赵仁杰,赵欣仪,钟世虎 & 许文立.(2023).征管模式、征管技术与税收治理
* ——来自取消税管员固定管户的证据.经济研究,58(09),133-151. ****
**********************************
use 强平衡面板08-15.dta,clear

gl covars_city "i.year#( c.lnpergdp c.industry c.thirdindus c.fpresure c.taxrate c.lnlandincome)"

* cDCH估计量
did_multiplegt ETR newid2 year policy if policyjs==0, cluster(citycode) breps(20)

did_multiplegt_dyn ETR newid2 year policy if policyjs==0,effects(3) placebo(3) cluster(citycode)

* 传染偏误稳健估计量policy
did_multiplegt_dyn ETR newid2 year policy,effects(3) placebo(3) cluster(citycode) breps(20) if_first_diff(d.policyjs==0) trends_nonparam(policyjs) always_trends_nonparam

* 传染偏误稳健估计量policyjs

did_multiplegt_dyn ETR newid2 year policyjs, cluster(citycode) breps(20) controls($covars_city) if_first_diff(d.policy==0) trends_nonparam(policy) always_trends_nonparam

did_multiplegt_dyn ETR newid2 year policyjs,effects(4) placebo(3) long cluster(citycode) breps(20) if_first_diff(d.policy==0) trends_nonparam(policy) always_trends_nonparam
